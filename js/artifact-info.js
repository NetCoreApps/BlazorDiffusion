...
export default {
    ...
    props:['artifactId'],
    setup(props) {
        const hasLiked = computed(() => AppData.UserData.liked)
        const { api, error, loading } = useClient()
        const { artifactId } = props

        function unlikeArtifact() {
            AppData.UserData.liked = false
            api(new DeleteArtifactLike({ artifactId }))
                .then(r => {
                    if (!r.succeeded) {
                        AppData.UserData.liked = true
                    }
                })
        }
